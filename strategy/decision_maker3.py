...
from strategy.propagator import Propagator
from public import * 
...
class PostflopDecisionMaker():#{{{
    ...
    def update_stats(self, actor, action):#{{{
        ...
        if action < SB:
            ...
            if self.betting[actor] / self.pot < 0.2:
                action_name = 'check'
            else:
                action_name = 'bet'
        ...
        if self.last_mover == actor:
            if max(self.betting) == self.betting[actor]:#{{{
                ...
                for n1, c1, n2, c2, prob in nodes_of_tree(self.small_stats[self.opponent[actor]], 4):
                    ...
                    if self.source != 'ps' and actor != 0 and self.small_stats[actor][n1][c1][n2][c2] > 0.1:
                        print n1, c1, n2, c2, '\t',\
            ...
            if max(self.betting) == self.betting[actor]:#{{{
                ...
                for n1, c1, n2, c2, prob in nodes_of_tree(self.small_stats[self.opponent[actor]], 4):
                    ...
                    if self.source != 'ps' and actor != 0 and self.small_stats[actor][n1][c1][n2][c2]:
                        print n1, c1, n2, c2, '\t',\
                                round(vdt['bet'][n1][c1][n2][c2], 2), '\t',\
                                round(vdt['check'][n1][c1][n2][c2], 2)
                    vdt['check'][n1][c1][n2][c2] = max(0.4, vdt['check'][n1][c1][n2][c2])
...
#}}}
    ...
    def flop_strategy(self):#{{{
        print 'starting'
        status_package = {}
        status_package['hero'] = 0
        status_package['betting'] = self.betting
        status_package['stack'] = self.stack
        status_package['pot'] = self.pot
        status_package['active'] = self.active
        status_package['actor'] = 0
        status_package['stats'] = self.stats
        status_package['stage'] = self.stage
        status_package['cards'] = self.cards
        status_package['button'] = self.button
        status_package['last_better'] = self.last_better
        status_package['wcts'] = self.flop_wcts
        status_package['dummy_action_ep'] = self.dummy_action_ep
        status_package['dummy_action_lp'] = self.dummy_action_lp
        propagator = Propagator(status_package)
        print propagator.propagate()
        raw_input('done propagating')
