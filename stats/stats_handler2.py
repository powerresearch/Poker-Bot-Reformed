...
class StatsHandler():
    ...
    @staticmethod
    def map_power_to_prob(power, left_vertex, right_vertex, slope=1.5, min_prob=0.2):
        if power < left_vertex:#{{{
            prob = 1 - (left_vertex-power)*slope
        elif power > right_vertex:
            prob = 1 - (power-right_vertex)*slope
        else:
            prob = 1
        if prob < 0.1:
            prob = 0.1
    ...
    def postflop_update(self, actor, postflop_status_list, cards, stage):
        wct = self.game_driver.win_chance_table
                ...
                for col1 in xrange(1, 5):
                    for num2 in xrange(2, 15):
                        for col2 in xrange(1, 5):
                            ...
                            prob = self.map_power_to_prob(power, vertex_l, vertex_r)
                ...
                for col1 in xrange(1, 5):
                    for num2 in xrange(2, 15):
                        for col2 in xrange(1, 5):
                            ...
                            vertex_r = 1
                            prob = self.map_power_to_prob(power, vertex_l, vertex_r, slope=3)
#                           print big_card, small_card, can_beat, vertex_l, vertex_r, prob
                            self.stats[actor][num1][col1][num2][col2] = \
                                    self.stats_stable[actor][num1][col1][num2][col2] * prob#}}}
#                            to_print.append([[num1, col1], [num2, col2],\
...
#}}}
    ...
    def postflop_big_update(self, cards, active):
        to_del1 = list()#{{{
        to_del2 = list()
        players = [c for c in xrange(6) if active[c] > 0]
        for i in players:
            for n1 in self.stats[i]:
                for c1 in self.stats[i][n1]:
                    if [n1, c1] in cards[2:]:
                        to_del1.append([i, n1, c1])
                        continue
                    for n2 in self.stats[i][n1][c1]:
                        for c2 in self.stats[i][n1][c1][n2]:
                            if [n2, c2] in cards[2:]:
                                to_del2.append([i, n1, c1, n2, c2])
        for combo in to_del1:
            del self.stats[combo[0]][combo[1]][combo[2]]
        for combo in to_del2:
            del self.stats[combo[0]][combo[1]][combo[2]][combo[3]][combo[4]]
        ...
        for num1 in xrange(2, 15):
            ...
            for col1 in xrange(1, 5):
                ...
                for num2 in xrange(2, 15):
                    ...
                    if len(big_table[num1][col1][num2]) == 0:
                        del big_table[num1][col1][num2]
        return big_table#}}}
