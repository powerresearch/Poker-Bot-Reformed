...
class StatsHandler():
    ...
    def preflop_update(self, action, betting, bet_round, people_play, last_better):
        ...
        if bet_round == 1:
            ...
            top_n = [self.data_manager.get_item(player, u'pfr'), \
                    self.data_manager.get_item(player, u'vpip')]
        elif bet_round == 2:
            ...
            if last_better == player:
                ...
                top_n = [self.data_manager.get_item(player, u'4B'),\
                        self.data_manager.get_item(player, u'3B')]
        elif bet_round >= 4:
            if last_better == player:
                top_n = self.data_manager.get_item(player, u'4B')
                para = 'open'
            else:
                top_n = [1.5, self.data_manager.get_item(player, u'4B')]
                para = 'close'
        if type(top_n) == float:
            small_table = self.get_preflop_prob(top_n*100, para)
        else:
            small_table = self.get_preflop_prob(map(lambda x:x*100, top_n), para)
    ...
    def get_preflop_prob(top_n, arg):
        if arg == 'close':#{{{
            n = range(min(int(top_n[1]), int(top_n[0])+3), int(top_n[1])+1, 1)
        else:
    ...
    def get_preflop_prob_pointy(top_n, arg):
        ...
        for point_index in xrange(len(cut_points)-1, -1, -1):
            if cut_points[point_index] < top_n:
                coef[point_index] = 1.0 / (100+25*pow(abs(cut_points[point_index]-top_n), 2))
            else:
